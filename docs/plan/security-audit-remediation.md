...
- **Deploy scripts were not dry-run against a live fork.** `forge`'s fork backend times out in this environment against every provider tried (publicnode, rpc.sepolia.org, 1rpc), with no proxy vars set, while `cast` reaches the same URLs in ~1s. Script logic *was* exercised against a local `anvil` replica seeded from live bytecode and storage. **Run `forge script` dry-runs from an unrestricted machine before shipping.**
...
- `docs/` is gitignored (`.gitignore:14`), so this document and the runbook are untracked by design.
